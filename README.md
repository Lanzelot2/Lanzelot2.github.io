<link rel="stylesheet" href="style.css">

<div class="tutorial">

# Modrinth: Modpack installieren (EbeluCraft)

<p class="lead">
Auf dem EbeluCraft-Server kann man zwar auch komplett ohne Mods spielen, aber für Performance und Features (z. B. Ingame-Voice-Chat) ist das Modpack klar empfohlen.
</p>

<div class="callout">
<strong>Kurz gesagt:</strong> Modrinth App installieren → einloggen → RAM einstellen → Modpack importieren → Minecraft starten → Server beitreten.
</div>

## Voraussetzungen

<ul class="checklist">
  <li>Minecraft (Java Edition) ist installiert</li>
  <li>Windows / macOS / Linux</li>
  <li><strong>macOS:</strong> ggf. Mikrofon-Berechtigung später erlauben (Voice-Chat)</li>
</ul>

## Schritt-für-Schritt

<div class="steps">

<section class="step">
  <div class="step__head">
    <div class="step__num"></div>
    <h3>Modrinth App installieren</h3>
  </div>
  <div class="step__grid">
    <div class="step__text">
      Geht auf die offizielle Seite und ladet euch die App runter: <a href="https://modrinth.com/app" target="_blank" rel="noopener">modrinth.com/app</a>.
      Installiert sie wie jede andere App.
    </div>
    <div class="step__media"></div>
  </div>
</section>

<section class="step">
  <div class="step__head">
    <div class="step__num"></div>
    <h3>In euren Minecraft-Account einloggen</h3>
  </div>
  <div class="step__grid">
    <div class="step__text">
      Öffnet Modrinth, klickt links auf <strong>Account</strong> und meldet euch an.
      Ihr werdet kurz in den Browser weitergeleitet – folgt dort den Schritten.
      <br><br>
      Tipp: Auf dem Handy könnt ihr Screenshots antippen (sie öffnen sich dann groß).
    </div>
    <div class="step__media">
      <a href="https://github.com/user-attachments/assets/51bafdc6-aa0f-4ce8-b0d5-74a8d97f3337" target="_blank" rel="noopener">
        <img alt="Account-Login" src="https://github.com/user-attachments/assets/51bafdc6-aa0f-4ce8-b0d5-74a8d97f3337" loading="lazy">
      </a>
    </div>
  </div>
</section>

<section class="step">
  <div class="step__head">
    <div class="step__num"></div>
    <h3>Einstellungen öffnen</h3>
  </div>
  <div class="step__grid">
    <div class="step__text">
      Öffnet die Einstellungen wie im Screenshot markiert.
    </div>
    <div class="step__media">
      <a href="https://github.com/user-attachments/assets/a9cd1c22-6fc9-4fc0-938a-7c399356f1ca" target="_blank" rel="noopener">
        <img alt="Einstellungen öffnen" src="https://github.com/user-attachments/assets/a9cd1c22-6fc9-4fc0-938a-7c399356f1ca" loading="lazy">
      </a>
    </div>
  </div>
</section>

<section class="step">
  <div class="step__head">
    <div class="step__num"></div>
    <h3>RAM (Arbeitsspeicher) einstellen</h3>
  </div>
  <div class="step__grid">
    <div class="step__text">
      Geht zu <strong>Default Installation</strong> und stellt ein, wie viel RAM Minecraft nutzen darf.
      Das hängt von eurem PC ab:
      <div class="ram-grid">
        <div class="ram-card"><strong>Unter 8 GB RAM</strong><span>2–3 GB</span></div>
        <div class="ram-card"><strong>8 GB RAM</strong><span>3–4 GB</span></div>
        <div class="ram-card"><strong>16+ GB RAM</strong><span>6–max. 10 GB (zu viel kann sogar schaden)</span></div>
      </div>
    </div>
    <div class="step__media">
      <a href="https://github.com/user-attachments/assets/78cd1633-3a9b-4440-bdb5-b45796b5b10b" target="_blank" rel="noopener">
        <img alt="RAM einstellen" src="https://github.com/user-attachments/assets/78cd1633-3a9b-4440-bdb5-b45796b5b10b" loading="lazy">
      </a>
    </div>
  </div>
</section>

<section class="step">
  <div class="step__head">
    <div class="step__num"></div>
    <h3>Modpack importieren</h3>
  </div>
  <div class="step__grid">
    <div class="step__text">
      Klickt links auf das <strong>+</strong> und wählt <strong>From File</strong>.
      Alternativ könnt ihr das Modpack auch per Drag & Drop in die App ziehen.
    </div>
    <div class="step__media">
      <a href="https://github.com/user-attachments/assets/298a34b9-8969-4197-baa4-e582a402f71c" target="_blank" rel="noopener">
        <img alt="From File auswählen" src="https://github.com/user-attachments/assets/298a34b9-8969-4197-baa4-e582a402f71c" loading="lazy">
      </a>
      <a href="https://github.com/user-attachments/assets/9244bf08-76c1-489e-a56c-faa2f4f77c59" target="_blank" rel="noopener">
        <img alt="Datei auswählen" src="https://github.com/user-attachments/assets/9244bf08-76c1-489e-a56c-faa2f4f77c59" loading="lazy">
      </a>
    </div>
  </div>
</section>

<section class="step">
  <div class="step__head">
    <div class="step__num"></div>
    <h3>Minecraft starten und Server beitreten</h3>
  </div>
  <div class="step__grid">
    <div class="step__text">
      Nach dem Import findet ihr das Modpack links in eurer Library.
      Wenn ihr mit der Maus drüber hovert, erscheint ein <strong>Play-Button</strong>.
      <br><br>
      Alternativ könnt ihr ins Profil gehen und dort den großen Play-Button drücken.
      Im <strong>Worlds</strong>-Tab könnt ihr oft auch direkt beim Server auf Play klicken.
      <br><br>
      Wenn ihr schon mal auf dem Server wart, taucht er später häufig als Schnellstart-Option auf der Startseite auf.
    </div>
    <div class="step__media">
      <a href="https://github.com/user-attachments/assets/c68ec307-0630-401f-aceb-bfed183b251f" target="_blank" rel="noopener">
        <img alt="Play-Button in der Library" src="https://github.com/user-attachments/assets/c68ec307-0630-401f-aceb-bfed183b251f" loading="lazy">
      </a>
      <a href="https://github.com/user-attachments/assets/0152f048-30e1-4855-996a-5a2820dd966c" target="_blank" rel="noopener">
        <img alt="Server über Worlds-Tab starten" src="https://github.com/user-attachments/assets/0152f048-30e1-4855-996a-5a2820dd966c" loading="lazy">
      </a>
      <a href="https://github.com/user-attachments/assets/f7edf3b0-975c-4a65-b28c-fbc07a8e67db" target="_blank" rel="noopener">
        <img alt="Schnellstart auf der Startseite" src="https://github.com/user-attachments/assets/f7edf3b0-975c-4a65-b28c-fbc07a8e67db" loading="lazy">
      </a>
    </div>
  </div>
</section>

</div>

## Zusatz: Mods verwalten

Im Profil unter <strong>Content</strong> seht ihr alle installierten Mods. Dort könnt ihr Mods updaten, deaktivieren (falls etwas stört) oder optional wieder aktivieren.

<details class="faq">
  <summary>Wenn Minecraft nicht startet / laggt</summary>

  Prüft zuerst die RAM-Einstellung (Schritt 4). Zu wenig oder viel zu viel kann Probleme machen.  
  Schließt außerdem andere RAM-fressende Programme (viele Browser-Tabs, Discord-Streams, etc.).  
  macOS: Mikrofon-Berechtigungen prüfen (für Voice-Chat).

</details>

</div>
